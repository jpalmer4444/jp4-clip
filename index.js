...
let PSTiffImageWriter = require("./lib/PSTiffImageWriter").PSTiffImageWriter;
let LOCAL = require("./lib/local");

//(header, ifd0, inputfilename, outputfilename, callback)
    ...
    tiffByteReader.read(ifdOffset, path, function (err, ifd0) {
        if (err) {
            ...
            if (photoshopImageResourceBlock) {
                ...
                let tiffWriter = new PSTiffImageWriter(tiffByteReader.header, ifd0, path, '/tmp/test.tif', function (err, results) {
                    if (err) {
                        LOG.error(err);
                        CALLBACK(err, err.stack);
                        process.exit();
