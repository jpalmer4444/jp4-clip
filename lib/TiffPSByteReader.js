...
class TiffPSByteReader {
    ...
    read(startOffset, file, cb) {
        ...
        if (!startOffset) {
            reads.push(function(callback){
                (function(_TiffByteReader) {
                    let br = new BinaryReader(file);
                            br.seek(startOffset, function (error) {
                                if (error) {
                                    close(br, error, callback);
                                }
                                br.read(8, function (error, bytes, bytesRead) {
                                    if (error) {
                                        close(br, error, callback);
                                    } else {
        ...
        reads.push((ifdOffset, callback) => {
            ((_TiffByteReader) => {
                let br  = new BinaryReader(file);
                        br.seek(ifdOffset, function (error) {
                            if (error) {
                                close(br, error, callback);
                            }
            ...
            ((_TiffByteReader) => {
                let br = new BinaryReader(file);
                        br.seek(ifdOffset, function (error) {
                            ...
                            br.read((2 + (ifdcount * 12)) + 4, function (error, bytes, bytesRead) {
                                if (error) {
                                    close(br, error, callback);
                                } else {
            ...
            if (err) {
                ...
                async.parallel(fillIFDs, function (err, results) {
                    if (err) {
                        ...
                        cb(null, startOffset === 0 ? _TiffByteReader.IFDS[0] : _TiffByteReader.IFDS[_TiffByteReader.IFDS.length - 1]);
                    }
                });
            }
        });
