...
class PSImageResourceBlock{
        ...
        while(true){
            try{
                ...
                if(this._index !== -1){
                    ...
                    this._index += 4;
                    let identifier = Buffer.alloc(2, bytes.slice(this._index, this._index + 2))['readUIntBE'](0, 2, false);
                    this._index+=2;
    ...
    getPhotoshopEntry(id){
        for(let photoshopentry of this._resources.values()){
            if(photoshopentry.identifier === id)
                return photoshopentry;
        }
    }
